...
const { SyncBailHook, HookMap, SyncWaterfallHook } = require("tapable");
const vm = require("vm");
...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto") {
		...
		this.hooks = Object.freeze({
			...
			/** @type {SyncWaterfallHook<[Object | null, ExpressionNode]>} */
			commentOptions: new SyncWaterfallHook(["options", "expression"])
		});
	...
	parseCommentOptions(range, expr) {
		const comments = this.getComments(range);
		if (comments.length === 0) {
			return {
				errors: null,
				options: this.hooks.commentOptions.call(null, expr)
			};
		}
		let options = {};
		let errors = [];
		...
		this.hooks.commentOptions.call(options, expr);
